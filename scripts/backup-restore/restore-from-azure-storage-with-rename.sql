...
SELECT @StorageAccountName = 'sciencebitch';
...
SELECT @TSQL = 'RESTORE DATABASE [' + @DatabaseNewName + '] FROM URL = ''https://' + @StorageAccountName + '.blob.core.windows.net/' + @ContainerName+ '/'+ @BakFileName + '_0.bak'''
...
BEGIN
    SELECT @TSQL = @TSQL + ' WITH MOVE ''' + @DatabaseName + ''' TO ''' + @DestinationFolderData + @DatabaseNewName + '.mdf'''

    IF(@DestinationFolderLog != '')
    SELECT @TSQL = @TSQL + ' ,MOVE ''' + @DatabaseName + '_log'' TO ''' + @DestinationFolderLog + @DatabaseNewName + '_log.ldf'''
